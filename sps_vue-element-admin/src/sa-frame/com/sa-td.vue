<template>
  ...
  <el-table-column v-if="$slots.default || $scopedSlots.default" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <slot :row="s.row" :index="s.index" />
    </template>
  </el-table-column>
  <!-- selection框 -->
  <el-table-column v-else-if="type == 'selection'" type="selection" :width="width || '45px'" :min-width="minWidth" />
  <!-- 普通td -->
  ...
  <el-table-column v-else-if="type == 'icon'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <i v-if="s.row[prop]" :class="s.row[prop]" style="font-size: 1.3em;" />
      <span v-else>{{ not }}</span>
    </template>
  </el-table-column>
  <!-- img -->
  ...
  <el-table-column v-else-if="type == 'audio' || type == 'video' || type == 'file'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <el-link v-if="!sa.isNull(s.row[prop])" type="info" :href="s.row[prop]" target="_blank">预览</el-link>
      <span v-else>{{ not }}</span>
    </template>
  </el-table-column>
  <!-- img-list -->
  <el-table-column v-else-if="type == 'img-list'" :label="name" :width="width" :min-width="minWidth || '120px'" show-overflow-tooltip>
    <template slot-scope="s">
      <div v-if="s.row[prop]" style="cursor: pointer;" @click="sa.showImageList(value_to_arr(s.row[prop]))">
  ...
  <el-table-column v-else-if="type == 'link'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <el-link v-if="!sa.isNull(s.row[prop])" type="primary" :href="s.row[prop]" target="_blank">{{ s.row[prop] }}</el-link>
      <div v-else>无</div>
    </template>
  </el-table-column>
  <!-- link-btn -->
  <el-table-column v-else-if="type == 'link-btn'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <el-link v-if="!sa.isNull(s.row[prop])" type="primary" @click="$emit('click', s)">{{ s.row[prop] }}</el-link>
      <div v-else>无</div>
    </template>
  </el-table-column>
  ...
  <el-table-column v-else-if="type == 'money'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <b v-if="!sa.isNull(s.row[prop])" class="c-price">￥{{ s.row[prop] }}</b>
      <div v-else>无</div>
    </template>
  </el-table-column>
  <!-- 钱 price-f (单位 分) -->
  <el-table-column v-else-if="type == 'money-f'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <b v-if="!sa.isNull(s.row[prop])" class="c-price">￥{{ s.row[prop] / 100 }}</b>
      <div v-else>无</div>
    </template>
  </el-table-column>
  <!-- 显示枚举 j、num -->
  <el-table-column v-else-if="type == 'enum' || type == 'j'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <b v-for="j in jvList" v-if="s.row[prop] == j.key" :key="j.key" :style="{color: j.color || '#606266'}">{{ j.value }}</b>
    </template>
  </el-table-column>
  <!-- switch 开关 -->
  <el-table-column v-else-if="type == 'switch'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <el-switch 
        v-if="jvList.length >= 2" v-model="s.row[prop]" 
      ...
      />
      <span v-for="j in jvList" v-if="s.row[prop] == j.key" :key="j.key" :style="{color: '#999'}">{{ j.value }}</span>
    </template>
  </el-table-column>
  <!-- rate 评分 -->
  <el-table-column v-else-if="type == 'rate'" :label="name" :width="width" :min-width="minWidth">
    <template slot-scope="s">
      <el-rate v-if="!sa.isNull(s.row[prop])" :value="s.row[prop] <= 5 ? s.row[prop] : 5" show-text disabled />
      <div v-else>无</div>
    </template>
  </el-table-column>
  <!-- date 日期 -->

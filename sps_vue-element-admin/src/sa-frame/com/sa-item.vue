...
<template>
	...
	<div class="fast-btn" v-else-if="type == 'fast-btn'">
		<el-button type="primary" plain icon="el-icon-plus" @click="sa.currView().add()" v-if="showBtns.indexOf('add') != -1">新增</el-button>
		<el-button type="success" plain icon="el-icon-view" @click="sa.currView().getBySelect()" v-if="showBtns.indexOf('get') != -1">查看</el-button>
		<el-button type="danger" plain icon="el-icon-delete" @click="sa.currView().deleteByIds()" v-if="showBtns.indexOf('delete') != -1">删除</el-button>
		<el-button type="warning" plain icon="el-icon-download" @click="sa.exportExcel()" v-if="showBtns.indexOf('export') != -1">导出</el-button>
		<el-button plain  icon="el-icon-refresh"  @click="sa.tagsView.f5CurrView()" v-if="showBtns.indexOf('reset') != -1">重置</el-button>
		<slot></slot>
	</div>
	<!-- 分页组件 -->

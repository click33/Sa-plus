...
(function(){
	// 公司开发环境
	var cfg_dev = {
		api_url: 'http://127.0.0.1:8989',	// 所有ajax请求接口父地址
	}
...
(function(){
	...
	sa.ajax = function(url, type, header, data, isCookie, successFn, errorFn){
		...
		return $.ajax({
			...
			// 跨域处理 
			xhrFields: {
				withCredentials: isCookie // 携带跨域cookie
			},
			crossDomain: true,
...
(function() {
	...
	// 显示一个iframe 
	// 参数: 标题，地址，宽，高 , 点击遮罩是否关闭, 默认false 
	me.showIframe3 = function(title, url, w, h, shadeClose) {
		// 参数修正
		w = w || '95%'; 
		h = h || '95%'; 
		shadeClose = (shadeClose === undefined ? false : shadeClose);
...
(function(){
	...
	me.openAdminInfo = function(id, username) {
		...
		sa.showIframe(title, '../sf-admin/admin-info.html?id=' + id, '700px', '600px');
	}
